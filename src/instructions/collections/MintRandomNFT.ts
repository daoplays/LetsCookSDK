...
export const GetMintRandomNFTInstruction = async (
    ...
    const token_mint = collection.keys[CollectionKeys.MintAddress];
    const mint_account = await getMintData(connection, collection.keys[CollectionKeys.MintAddress].toString());
    ...
    const nft_mint_keypair = new Keypair();
    const nft_mint_account = nft_mint_keypair.publicKey;
